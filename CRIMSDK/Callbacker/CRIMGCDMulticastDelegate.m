...
@interface CRIMGCDMulticastDelegateNode : NSObject {
@private
...
@interface CRIMGCDMulticastDelegate ()
{
...
@interface CRIMGCDMulticastDelegateEnumerator ()
{
	NSUInteger numNodes;
...
@implementation CRIMGCDMulticastDelegate
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	CRIMGCDMulticastDelegateNode *node =
	    [[CRIMGCDMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	{
		CRIMGCDMulticastDelegateNode *node = delegateNodes[i - 1];
		...
		if (delegate == nodeDelegate)
		{
			...
			{
				// Recall that this node may be retained by a CRIMGCDMulticastDelegateEnumerator.
...
{
	for (CRIMGCDMulticastDelegateNode *node in delegateNodes)
	{
		node.delegate = nil;
		#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
		node.unsafeDelegate = nil;
...
- (CRIMGCDMulticastDelegateEnumerator *)delegateEnumerator
{
	return [[CRIMGCDMulticastDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
@implementation CRIMGCDMulticastDelegateNode
...
@implementation CRIMGCDMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr
{
	...
	{
		CRIMGCDMulticastDelegateNode *node = delegateNodes[currentNodeIndex];
		currentNodeIndex++;
